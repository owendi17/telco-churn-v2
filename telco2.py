...
if best_model_name in results and 'feature_importance' in results[best_model_name]:
    top_features = results[best_model_name]['feature_importance'].head(3)
    for idx, (feat, imp) in enumerate(zip(top_features['feature'], top_features['importance']), 1):
        summary += f"{idx}. {feat} ({imp:.4f})\n"
else:
    summary += "(This model type doesn't have feature importance)\n"
